...
def default_target_root(claude_home: Path | None = None) -> Path:
    home = claude_home or Path(os.environ.get("CLAUDE_HOME", Path.home() / ".claude"))
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Install Autocite into Claude Code skills.")
    parser.add_argument("--source", type=Path, default=REPO_ROOT)
    parser.add_argument("--target", type=Path, help="Claude skills root. Defaults to $CLAUDE_HOME/skills or ~/.claude/skills.")
...
def main() -> int:
    ...
    try:
        ...
        return 1
    print(f"[OK] Installed Autocite for Claude Code: {installed}")
    return 0
